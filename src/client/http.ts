...
import { EntgeltatlasNetworkError } from "./errors.js";
...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new EntgeltatlasNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${request.url}`));
      return;
    }
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          ...
          if (maxBytes !== undefined && received > maxBytes) {
            aborted = true;
            res.destroy();
            reject(new EntgeltatlasNetworkError(`Response exceeded maxResponseBytes (${maxBytes})`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        res.on("error", (err) => {
          ...
          reject(new EntgeltatlasNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      req.setTimeout(request.timeoutMs, () => {
        req.destroy(new EntgeltatlasNetworkError(`Request timed out after ${request.timeoutMs}ms`));
      });
    }
