...
import { EntgeltatlasNetworkError } from "../src/client/errors.js";
  ...
  await withServer(
    ...
    async (baseUrl) => {
      const resp = await nodeHttpTransport({
        method: "GET",
        url: `${baseUrl}/infosysbub/entgeltatlas/pc/v1/entgelte/84304`,
      });
      assert.equal(resp.status, 200);
      assert.deepEqual(JSON.parse(resp.body.toString("utf8")), {
        path: "/infosysbub/entgeltatlas/pc/v1/entgelte/84304",
      });
    },
  );
});

test("rejects an unsupported protocol with EntgeltatlasNetworkError", async () => {
  ...
  await withServer(
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof EntgeltatlasNetworkError && /timed out after 300ms/.test(err.message),
      );
